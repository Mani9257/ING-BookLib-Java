...
public class LoginServiceImpl implements LoginService {
	...
	/**
	 * @author Sucheta This method is intended to login the user by
	 *         taking userid and password for login
	 * @param loginDto is the input request which includes userid and
	 *                 password
	...
	public LoginResponseDto loginUser(UserLoginDto loginDto) {
		...
		LOGGER.info("inside login");
		 if(user == null) throw new
		  UserDoesNotExistException("user doesnt exist");
		
		  
		 LoginResponseDto loginResponseDto1 = new LoginResponseDto();
		 loginResponseDto1.setUserId(user.getUserId());
		 loginResponseDto1.setStatusCode(constants.SUCCESS_STATUS_CODE);
		 loginResponseDto1.setMessage(constants.SUCCESS_STATUS_MESSAGE);
